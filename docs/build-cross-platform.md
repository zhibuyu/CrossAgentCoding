# CrossAgentCoding 跨平台构建指南

## 概述

CrossAgentCoding 在不同平台上以**不同的实现与分发方式**分开打包：
...
构建产物：
- `release/CrossAgentCoding-0.0.1.dmg` — 拖入 Applications 即用的磁盘映像

### 安装到系统

```bash
open release/CrossAgentCoding-0.0.1.dmg   # 挂载后把 CrossAgentCoding.app 拖入 Applications
```
...
```bash
# 直接双击 /Applications/CrossAgentCoding.app  → TUI 菜单

# 或当作 CLI（同一份 Node 代码）：
node /Applications/CrossAgentCoding.app/Contents/Resources/app/cac.mjs env tools
...
构建产物：
- `release/crossagentcoding` — CLI 启动脚本
- `release/CrossAgentCoding.desktop` — 桌面入口文件

### 安装

```bash
sudo ln -sf "$(pwd)/release/crossagentcoding" /usr/local/bin/crossagentcoding
cp release/CrossAgentCoding.desktop ~/.local/share/applications/
```
...
- 输出：`release/CrossAgentCoding.exe`
